 ...
 * spNFTs are yield-generating positions when the NFTPool contract has allocations from the Kayen Master
 */
contract NFTPool is ReentrancyGuard, INFTPool, ERC721("Kayen staking position NFT", "spNFT") {
  ...
  struct StakingPosition {
    ...
    uint256 boostPoints; // Allocated xKAYEN from yieldboost contract (optional)
    uint256 totalMultiplier; // lockMultiplier + allocated xKAYEN boostPoints multiplier
    uint256 pendingXKayenRewards; // Not harvested xKayen rewards
    uint256 pendingKayenRewards; // Not harvested Kayen rewards
  }
  ...
  IKayenMaster public master; // Address of the master
  ...
  IERC20 private _kayenToken; // KayenToken contract's address
  IXKayenToken private _xKayenToken; // XKayenToken contract's address
  ...
  uint256 private _maxBoostMultiplier = 10000; // 100%, Max boost that can be earned from xKayen yieldBooster

  uint256 private constant _TOTAL_REWARDS_SHARES = 10000; // 100%, high limit for xKayenRewardsShare
  uint256 public xKayenRewardsShare = 8000; // 80%, directly defines kayenShare with the remaining value to 100%
  ...
  function initialize(IKayenMaster master_, IERC20 kayenToken, IXKayenToken xKayenToken, IERC20 lpToken) external {
    require(msg.sender == factory && !initialized, "FORBIDDEN");
    _lpToken = lpToken;
    master = master_;
    _kayenToken = kayenToken;
    _xKayenToken = xKayenToken;
    initialized = true;

    // to convert KAYEN to xKAYEN
   _kayenToken.approve(address(_xKayenToken), type(uint256).max);
  }
  ...
  event SetXKayenRewardsShare(uint256 xKayenRewardsShare);
  ...
  function getPoolInfo() external view override returns (
    address lpToken, address kayenToken, address xKayenToken, uint256 lastRewardTime, uint256 accRewardsPerShare,
    ...
    return (
    address(_lpToken), address(_kayenToken), address(_xKayenToken), lastRewardTime, _accRewardsPerShare,
  ...
  function pendingRewards(uint256 tokenId) external view returns (uint256) {
    ...
    return position.amountWithMultiplier.mul(accRewardsPerShare).div(1e18).sub(position.rewardDebt)
      .add(position.pendingXKayenRewards).add(position.pendingKayenRewards);
  }
  ...
  /**
   * @dev Set the share of xKAYEN for the distributed rewards
   * The share of KAYEN will incidently be 100% - xKayenRewardsShare
   *
   * Must only be called by the owner
   */
  function setXKayenRewardsShare(uint256 xKayenRewardsShare_) external {
    _requireOnlyOwner();
    require(xKayenRewardsShare_ <= _TOTAL_REWARDS_SHARES, "too high");

    xKayenRewardsShare = xKayenRewardsShare_;
    emit SetXKayenRewardsShare(xKayenRewardsShare_);
  }
  ...
  function createPosition(uint256 amount, uint256 lockDuration) external nonReentrant {
    ...
    _stakingPositions[currentTokenId] = StakingPosition({
      ...
      pendingKayenRewards: 0,
      pendingXKayenRewards: 0
    });
   ...
   * Users should still be able to deallocate xKAYEN from the YieldBooster contract
   */
  ...
  function _harvestPosition(uint256 tokenId, address to) internal {
    ...
    // transfer rewards
    if (pending > 0 || position.pendingXKayenRewards > 0 || position.pendingKayenRewards > 0) {
      uint256 xKayenRewards = pending.mul(xKayenRewardsShare).div(_TOTAL_REWARDS_SHARES);
      uint256 kayenAmount = pending.add(position.pendingKayenRewards).sub(xKayenRewards);

      xKayenRewards = xKayenRewards.add(position.pendingXKayenRewards);

      // Stack rewards in a buffer if to is equal to address(0)
      if (address(0) == to) {
        position.pendingXKayenRewards = xKayenRewards;
        position.pendingKayenRewards = kayenAmount;
      }
      else {
        // convert and send xKAYEN + KAYEN rewards
        position.pendingXKayenRewards = 0;
        position.pendingKayenRewards = 0;

        if(xKayenRewards > 0) xKayenRewards = _safeConvertTo(to, xKayenRewards);
        // send share of KAYEN rewards
        kayenAmount= _safeRewardsTransfer(to, kayenAmount);

        // forbidden to harvest if contract has not explicitly confirmed it handle it
        _checkOnNFTHarvest(to, tokenId, kayenAmount, xKayenRewards);
      }
    }
  ...
  */
  ...
  function _safeRewardsTransfer(address to, uint256 amount) internal returns (uint256) {
    uint256 balance = _kayenToken.balanceOf(address(this));
    // cap to available balance
    if (amount > balance) {
      amount = balance;
    }
    _kayenToken.safeTransfer(to, amount);
    return amount;
  }

  /**
   * @dev Safe convert KAYEN to xKAYEN function, in case rounding error causes pool to not have enough tokens
   */
  function _safeConvertTo(address to, uint256 amount) internal returns (uint256) {
    ...
    if(amount > 0 ) _xKayenToken.convertTo(amount, to);
    return amount;
  }
   ...
   */
  function _checkOnNFTHarvest(address to, uint256 tokenId, uint256 kayenAmount, uint256 xKayenAmount) internal {
    address nftOwner = ERC721.ownerOf(tokenId);
    if (nftOwner.isContract()) {
      bytes memory returndata = nftOwner.functionCall(abi.encodeWithSelector(
          INFTHandler(nftOwner).onNFTHarvest.selector, msg.sender, to, tokenId, kayenAmount, xKayenAmount), "non implemented");
