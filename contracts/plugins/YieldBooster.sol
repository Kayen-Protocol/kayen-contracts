...
/*
 * This contract is a xKAYEN Usage (plugin) that can boost spNFTs' yield (staking positions on NFTPools) when it
 * receives allocations from the xKAYENToken contract
 */
contract YieldBooster is Ownable, ReentrancyGuard, IXKayenTokenUsage, IYieldBooster {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;
  using EnumerableSet for EnumerableSet.UintSet;

  IXKayenToken public immutable xKayenToken; // XKayenToken contract
  ...
  // User's position xKAYEN total allocation
  ...
  mapping(address => uint256) private _usersTotalAllocation; // User's xKAYEN total allocation
  mapping(address => uint256) private _poolsTotalAllocation; // Pool's total allocation
  uint256 public totalAllocation; // Contract's total xKAYEN allocation

  bool public forcedDeallocationStatus; // Authorize users to forcibly deallocate everything

  constructor(IXKayenToken xKayenToken_) {
  ...
  /**
   * @dev Checks if caller is the XKayenToken contract
   */
  modifier xKayenTokenOnly() {
    require(msg.sender == address(xKayenToken), "xKayenTokenOnly: caller should be xKayenToken");
    _;
  }
  ...
  /**
   * @dev Returns total xKAYEN allocated to this contract by "userAddress"
   */
  ...
  /**
   * @dev Returns total xKAYEN allocated to this contract by "poolAddress"
   */
  ...
  /**
   * @dev Returns allocated xKAYEN to "tokenId" spNFT from "poolAddress" NFTPool by "userAddress"
   */
  ...
  /**
   * Allocates "userAddress" user's "amount" of xKAYEN to this usage contract
   * "data" should contain tokenId and poolAddress
   *
   * Can only be called by XKayenToken contract, which is trusted to verify amounts
   */
  function allocate(address userAddress, uint256 amount, bytes calldata data) external override nonReentrant xKayenTokenOnly {
    (address poolAddress, uint256 tokenId) = abi.decode(data, (address, uint256));
    _allocate(userAddress, poolAddress, tokenId, amount);

    // allocated xKAYEN is added (as boost points) to spNFT
    INFTPool(poolAddress).boost(tokenId, amount);
  }

  /**
   * Deallocates "userAddress" user's "amount" of xKAYEN from this usage contract
   * "data" should contain tokenId and poolAddress
   *
   * Can only be called by XKayenToken contract, which is trusted to verify amounts
   */
  function deallocate(address userAddress, uint256 amount, bytes calldata data) external override nonReentrant xKayenTokenOnly {
    (address poolAddress, uint256 tokenId) = abi.decode(data, (address, uint256));
    _deallocate(userAddress, poolAddress, tokenId, amount);

    // should only be called if spNFT has not been burned, to avoid having stuck xKAYEN on it
    if(INFTPool(poolAddress).exists(tokenId)) {
      // allocated xKAYEN is removed (as boost points) from the spNFT
   ...
   * The pool should remove the allocated xKAYEN (boost points) from its own spNFT when calling this function
   */
  function deallocateAllFromPool(address userAddress, uint256 tokenId) external override nonReentrant {
    ...
    // update user's xKAYEN allocations balance
    xKayenToken.deallocateFromUsage(userAddress, amount);
  }

  /**
   * Deallocates msg.sender's "amount" of xKAYEN from XKayenToken, without adjusting this contract allocations balances
   *
  ...
  function forceDeallocate() external nonReentrant {
    ...
    uint256 amount = xKayenToken.usageAllocations(msg.sender, address(this));

    // update user's xKAYEN allocations balance
    xKayenToken.deallocateFromUsage(msg.sender, amount);
  }
  ...
  /**
   * @dev Returns multiplier that should be applied to a spNFT based on its boost points (allocated xKAYEN)
   *
   ...
   * 1% of the pool's allocated xKAYEN
   *
  ...
  /**
   * @dev Allocates "userAddress" user's "amount" of xKAYEN to "tokenId" spNFT of "poolAddress" pool
   */
  ...
  /**
   * @dev Deallocates "userAddress" user's "amount" of xKAYEN allocated to "tokenId" spNFT of "poolAddress" pool
   */
  function _deallocate(address userAddress, address poolAddress, uint256 tokenId, uint256 amount) internal {
    ...
    require(userPositionAllocation >= amount, "deallocate: not enough allocated xKayen");
