...
/*
 * This contract is used to distribute dividends to users that allocated xKayen here
 *
 ...
 * This distribution slot is the source of the dividends distribution to xKayen allocators during the current cycle
 *
 * This transfer from the pending slot to the distribution slot is based on cycleDividendsPercent and CYCLE_PERIOD_SECONDS
 *
 */
contract DividendsV2 is Ownable, ReentrancyGuard, IXKayenTokenUsage, IDividendsV2 {
  ...
  address public immutable xKayenToken; // xKayenToken contract

  mapping(address => uint256) public usersAllocation; // User's xKayen allocation
  ...
  constructor(address xKayenToken_, uint256 startTime_) {
    require(xKayenToken_ != address(0), "zero address");
  ...
  modifier xKayenTokenOnly() {
    require(msg.sender == xKayenToken, "xKayenTokenOnly: caller should be XKayenToken");
    _;
  }
  ...
  /**
   * Allocates "userAddress" user's "amount" of xKayen to this dividends contract
   *
   * Can only be called by xKayenToken contract, which is trusted to verify amounts
   * "data" is only here for compatibility reasons (IXKayenTokenUsage)
   */
  ...
  /**
   * Deallocates "userAddress" user's "amount" of xKayen allocation from this dividends contract
   *
   * Can only be called by xKayenToken contract, which is trusted to verify amounts
   * "data" is only here for compatibility reasons (IXKayenTokenUsage)
   */
  ...
  */
  ...
  function _updateDividendsInfo(address token) internal {
    ...
    // if no xKayen is allocated or initial distribution has not started yet
  ...
  function _harvestDividends(address token) internal {
    ...
    uint256 userXKayenAllocation = usersAllocation[msg.sender];
    uint256 pending = user.pendingDividends.add(
      userXKayenAllocation.mul(accDividendsPerShare).div(1e18).sub(user.rewardDebt)
    );

    user.pendingDividends = 0;
    user.rewardDebt = userXKayenAllocation.mul(accDividendsPerShare).div(1e18);
