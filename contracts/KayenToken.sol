 ...
 */
contract KayenToken is Ownable, ERC20, IKayenToken {
    ...
    ) ERC20("Kayen", "KAYEN") Ownable(msg.sender) {
        require(
    ...
    event UpdateAllocations(
        uint256 farmingAllocation,
        uint256 legacyAllocation,
    ...
    modifier onlyMaster() {
        require(
            ...
            "GrailToken: caller is not the master"
        );
        _;
    }
    ...
    function masterEmissionRate() public view override returns (uint256) {
        return
            emissionRate * (farmingAllocation + legacyAllocation) / ALLOCATION_PRECISION;
    }
    ...
    function updateAllocations(
        ...
        uint256 legacyAllocation_
    ) external onlyOwner {
        // apply emissions before changes
        emitAllocations();

        // total sum of allocations can't be > 100%
        uint256 totalAllocationsSet = farmingAllocation_ + legacyAllocation_;
        require(
