...
/*
 * This contract centralizes Kayen's yield incentives distribution.
 * Pools that should receive those incentives are defined here, along with their allocation.
 * All rewards are claimed from the KAYENToken contract.
 */
contract KayenMaster is Ownable, IKayenMaster {
  ...
  IKayenToken private immutable _kayenToken; // Address of the KAYEN token contract
  ...
  constructor(
    IKayenToken kayenToken_,
    uint256 startTime_
  ) {
    require(address(kayenToken_) != address(0), "KayenMaster: kayenToken cannot be set to zero address");
    require(_currentBlockTimestamp() < startTime_ && startTime_ >= kayenToken_.lastEmissionTime(), "KayenMaster: invalid startTime");
    _kayenToken = kayenToken_;
    startTime = startTime_; // Must be set with the same time as KayenToken emission start
  }
  ...
  /*
   * @dev Returns KayenToken address
   */
  function kayenToken() external view override returns (address) {
    return address(_kayenToken);
  }

  /*
   * @dev Returns KayenToken's emission rate (allocated to this contract)
   */
  function emissionRate() public view returns (uint256) {
    return _kayenToken.masterEmissionRate();
  }

  /**
   * @dev Returns current owner's address
   */
  function owner() public view virtual override(IKayenMaster, Ownable) returns (address) {
  ...
  function _safeRewardsTransfer(address to, uint256 amount) internal returns (uint256 effectiveAmount) {
    uint256 kayenBalance = _kayenToken.balanceOf(address(this));

    if (amount > kayenBalance) {
      amount = kayenBalance;
    }
  ...
  function _updatePool(address poolAddress) internal {
    ...
    if (allocPoint > 0 && INFTPool(poolAddress).hasDeposits()) {
      // calculate how much KAYEN rewards are expected to be received for this pool
      ...
      (rewards) = _kayenToken.claimMasterRewards(rewards);
