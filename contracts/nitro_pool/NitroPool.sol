...
contract NitroPool is ReentrancyGuard, Ownable, INFTHandler {
    ...
    using SafeERC20 for IXKayenToken;
    ...
    IKayenToken public kayenToken; // KAYENToken contract
    IXKayenToken public xKayenToken; // xKAYENToken contract
    ...
    constructor(
        IKayenToken kayenToken_, IXKayenToken xKayenToken_, address owner_, INFTPool nftPool_,
        IERC20 rewardsToken1_, IERC20 rewardsToken2_, Settings memory settings_
    ) {
        require(address(kayenToken_) != address(0) && address(xKayenToken_) != address(0) && owner_ != address(0)
     ...
     * "to" can be set to this address only if this contract is allowed to transfer xKAYEN
     */
    function onNFTHarvest(address operator, address to, uint256 tokenId, uint256 kayenAmount, uint256 xKayenAmount) external override isValidNFTPool(msg.sender) returns (bool) {
        address owner = tokenIdOwner[tokenId];
        require(operator == owner, "not allowed");

        // if not whitelisted, the NitroPool can't transfer any xKAYEN rewards
        require(to != address(this) || xKayenToken.isTransferWhitelisted(address(this)), "cant handle rewards");

        // redirect rewards to position's previous owner
        if (to == address(this)) {
            kayenToken.safeTransfer(owner, kayenAmount);
            xKayenToken.safeTransfer(owner, xKayenAmount);
        }
